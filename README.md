...
(6) 本项目提供了前后端分离的实现，包含注册与登录、大语言模型的选择、创建多个聊天窗口等功能。
...
- [x] **知识图谱可视化 (2025.12.29)**：集成了基于 vis-network 的交互式知识图谱，支持搜索、节点类型过滤及详细属性查看。
- [ ] NL2Cyhper
- [ ] 更多优化...
...
Frontend proxies `/api` to `http://127.0.0.1:8000` by default.
...
前端默认地址为 http://localhost:5173 ，后端接口为 http://127.0.0.1:8000
...
系统集成了全屏交互式知识图谱，方便直观地探索医疗知识：

- **实时搜索与高亮**：输入关键词（如“感冒”）即刻定位并高亮相关节点及邻居。
- **智能类型过滤**：支持按疾病、药品、症状、食物等维度进行关联探索，过滤结果更精准（邻居节点过滤逻辑）。
- **深层详情展示**：点击节点可侧边栏展示 Neo4j 中的完整属性（如疾病简介、预防措施等）。
- **高性能渲染**：采用 `vis-network` 手动优化渲染逻辑，支持百级节点流畅交互。
- **视觉区分**：不同实体类型采用不同颜色区分，搜索项特殊标记，视觉层次清晰。

详情参考：[知识图谱可视化板块讲解指南](./知识图谱可视化指南.md)

## 运行界面



系统包含登录注册界面，并支持用户与管理员两种身份。前端启动后即可访问登录页。
